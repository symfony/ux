 ...
 */
final class CropperjsExtensionAssetMapperTest extends TestCase
{
    public function testPrependsPathsWithAssetMapperBundle(): void
    {
        ...
        self::assertSame('@symfony/ux-cropperjs', array_values($config[0]['asset_mapper']['paths'])[0]);
    }

    public function testPrependsPathsWithLegacyFrameworkBundle(): void
    {
        $filesystem = new Filesystem();
        $directory = sys_get_temp_dir().'/cropperjs-framework-'.bin2hex(random_bytes(6));
    ...
    private function prepend(array $bundles): array
    {
        ...
        new CropperjsExtension()->prepend($container);
