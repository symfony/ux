...
function getPluralizationRule(number, locale) {
	...
	let _locale$1 = locale;
	if (locale === "pt_BR" || locale === "en_US_POSIX") return 0;
	_locale$1 = _locale$1.length > 3 ? _locale$1.substring(0, _locale$1.indexOf("_")) : _locale$1;
	switch (_locale$1) {
		case "af":
		case "bn":
		case "bg":
...
//#endregion
//#region src/formatters/intl-formatter.ts
/**
* @private
*
* @param id         The message id
...
function getLocale() {
	return _locale || document.documentElement.getAttribute("data-symfony-ux-translator-locale") || (document.documentElement.lang ? document.documentElement.lang.replace("-", "_") : null) || "en";
}
...
function trans(message, parameters = {}, domain = "messages", locale = null) {
	...
	if (typeof locale === "undefined" || null === locale) locale = getLocale();
	if (typeof message.translations === "undefined") return message.id;
	...
	if (_throwWhenNotFound) throw new Error(`No translation message found with id "${message.id}".`);
	return message.id;
}
