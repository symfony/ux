...
const setId = (element: Element, id: string | null): void => {
    if (id === null) {
...
export function executeMorphdom(
    ...
    childComponents: Iterable<{ element: HTMLElement; id: string | null }> = []
) {
    const externallyChangedIds = new Map<Element, string | null>();

    try {
        // Match using server IDs. Restore client IDs only after all matching and
        // focus restoration have finished, so Idiomorph's ID maps remain valid.
        ...
        // A parent tracker excludes child components. Use their stable server IDs
        // for preservation and keyed matching, then restore their client IDs too.
        for (const { element, id } of childComponents) {
                ...
                beforeNodeMorphed: (fromEl: Element, toEl: Element) => {
                    ...
                    if (fromEl instanceof HTMLElement && toEl instanceof HTMLElement) {
                        ...
                        if (externalMutationTracker.wasElementAdded(fromEl)) {
                            // A matching ID already represents this external element in the response.
                            // Retrying it would repeatedly match the same persistent node.
                            const id = fromEl.getAttribute('id');
                            if (id && id === toEl.getAttribute('id')) {
                                return false;
                            }

                            // Consume the externally-added old node without consuming the incoming node.
                            // Idiomorph iterates a live childNodes list, so the inserted marker makes
                            // it visit toEl again with the next old sibling.
                        ...
                        if (elementChanges) {
                            ...
                            if (externallyChangedIds.has(fromEl)) {
                                setId(toEl, serverId);
                            }
                        }
        ...
        for (const [element, id] of externallyChangedIds) {
            setId(element, id);
        }
    }
}
