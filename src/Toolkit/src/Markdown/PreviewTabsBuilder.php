...
/**
 * Builds the `Tabs(Preview: CodePreview, Code: FencedCode)` composition shared by the `::: example`
 * directive and the `{"preview": true}` fenced-code form. The code is put into the nodes verbatim,
 * so it is never re-parsed as markdown.
 *
...
final class PreviewTabsBuilder
{
     ...
     */
    public static function build(string $code, array $options = [], string $language = 'twig'): Tabs
    {
        $tabs = new Tabs();
