...
- `NativeSelect` renders a real `<select>`, so it keeps the platform's own picker, keyboard handling and type-ahead. This is the most accessible way to present a list of options.
...
- The chevron is drawn with `<twig:ux:icon>`, which renders `aria-hidden="true"`, and it is not focusable, so it never sits between the user and the control.
- `NativeSelect:OptGroup` renders a real `<optgroup>`, whose `label` is announced when moving into the group.
