...
The published styles supply light defaults and dark defaults for
``prefers-color-scheme: dark``. Applications that own their light and dark
modes can override the properties inside their own theme scope.

Browser Support
~~~~~~~~~~~~~~~

The optional stylesheets target Baseline Widely available, like the other
Symfony UX packages. They use cascade layers, ``color-mix()`` and ``:has()``.
These features affect only
